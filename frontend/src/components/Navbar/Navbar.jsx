import { Bell, BookOpenText, Settings } from 'lucide-react'
import React from 'react'

const Navbar = () => {
  return (
    <div className='flex justify-between p-4 shadow-xl bg-gray-50 items-center '>
        <div className='flex items-center cursor-pointer gap-2'>
            <div className='bg-blue-900 h-8 w-8 flex items-center justify-center rounded-md text-amber-50'> <BookOpenText /></div>
            <h1>Study Planner</h1>
        </div>
        <div>
            <ul className='flex gap-3 items-center cursor-pointer'>
                <li>Study.io</li>
                <li>Overview</li>
                <li>Team</li>
            </ul>
        </div>
        <div>
            <ul className='flex gap-3 items-center cursor-pointer'>
                <li><Bell/></li>
                <li><Settings/></li>
                <li className='w-10'>
                <img className='rounded-3xl' src="data:image/png;base64," alt="img"
                />
                </li>
            </ul>
        </div>
    </div>  
  )
}

export default Navbar